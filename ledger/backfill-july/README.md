...
| `confidence` | The confidence **as stated in July**, if one was stated. Never assigned retrospectively — a retro-fitted confidence poisons Loop 2's Brier inputs. Not stated → `null` (the schema permits null **only** for these pre-v3.2 backfill rows), with a NOT_FOUND note in `key_evidence_json`. |
...
| `system_version` | The version that actually produced the July decision — the July run predates v3.2, so **do not stamp 3.2.0 onto July decisions** (§16: outcomes must stay attributable to the system that produced them). The schema requires `major.minor.patch`: use the version the workbook records; where it records none, the convention is **`3.0.0`** (the July run operated under Investment Research System v3.0, 11 Jul 2026 — spec lineage), confirmed by Vyom at paste time. |
| `skill_versions_json`, `model_ids_json` | The July decisions predate the skills, and the schema requires both fields non-empty in every record. Conventions (schema-valid): `skill_versions_json` = `{"/log": "1.0"}` — the skill producing the *backfill record*, with a key_evidence note that the underlying July decision predates the skill registry; `model_ids_json` = `["NOT FOUND"]` where the July models were unrecorded — an honest sentinel, never a reconstructed ID. |
