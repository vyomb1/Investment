...
9. **Log** via [`/log`](../log/SKILL.md): one ledger record (`stage`: `redteam` — the single S5 stage; the pass is identified by `/redteam-rebuttal` in `skill_versions` and named in the verdict text) with verdict, confidence, key evidence, `system_version`, `skill_versions`, `model_ids` attached (§10). Unlogged = doesn't exist (Constitution 11).
