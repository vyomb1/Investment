...
8. **Log** via [`/log`](../log/SKILL.md): one ledger record (`stage`: redteam-blind) with verdict, confidence, key evidence, `system_version`, `skill_versions`, `model_ids` attached (§10). Unlogged = doesn't exist (Constitution 11).
...
- **No quotas (§2.5):** the three loss paths are the §7 task, not a padding license — if honest work surfaces only two evidence-tied paths, report two and say so; never manufacture a third from generic market risk.
